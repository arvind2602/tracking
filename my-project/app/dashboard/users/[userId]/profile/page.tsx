...
export default function UserProfileView({ params }: { params: Promise<{ userId: string }> }) {
    ...
    const handleDownloadIdCard = async () => {
        ...
        setIsProcessingImage(true); // This will trigger background removal in IDCardTemplate
        toast.loading("Processing image and generating ID Card...", { id: "download-toast" });
    };

    const handleTemplateReady = async () => {
        if (!isDownloading || !idCardRef.current) return;

        try {
            // Small delay to ensure rendering is perfect after image is set
            ...
            link.click();
            toast.success("ID Card downloaded", { id: "download-toast" });
        } catch (error) {
            console.error("Failed to download ID card", error);
            toast.error("Failed to download ID card", { id: "download-toast" });
        } finally {
        ...
        <div className="max-w-6xl mx-auto space-y-8 pb-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div style={{ position: "absolute", left: "-9999px", top: "-9999px" }}>
                <IDCardTemplate
                    ...
                    shouldProcess={isProcessingImage}
                />
            </div>
